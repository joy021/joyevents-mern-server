...
const router = express.Router();
const validateToken = require("../middlewares/validate-token");
const checkActiveUser = require("../middlewares/check-active-user");
const sendEmail = require("../helpers/send-email");
const BookingModel = require("../models/booking-model");
const UserModel = require("../models/user-model");
const EventModel = require("../models/event-model");
    ...
    if (refund.status === "succeeded") {
      ...
      const updatedTicketTypes = event.ticketTypes.map((ticketType) => {
        if (ticketType.name === ticketTypeName) {
          ...
          ticketType.available = Number(ticketType.available ?? ticketType.limit) + Number(ticketsCount);
        }
        return ticketType;
      });
...
// ✅ Get My Bookings (Optimized)
router.get("/my-bookings", validateToken, checkActiveUser, async (req, res) => {
  try {
    const bookings = await BookingModel.find({ user: req.user._id })
      .sort({ createdAt: -1 })
